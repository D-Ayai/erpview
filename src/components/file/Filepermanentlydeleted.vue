      ...
      <el-table-column label="永久删除">
        <template slot-scope="scope">
         <!-- <el-button type="warning" @click="openeditwin(scope.row.id)">删除</el-button>-->
           ...
           @confirm="delcartype"
         >
           <el-button type="danger" slot="reference" @click="delid=scope.row.id">永久删除</el-button>
         </el-popconfirm>
        </template>
      </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      delcartype() {  //根据id删除
        ...
        this.$axios.post("file/dele.action", params).then(function (response) {
